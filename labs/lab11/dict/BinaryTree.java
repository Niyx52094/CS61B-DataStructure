...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
	while(node!=null) {
	  	if(key.compareTo(node.entry.key)<0) {
	  		node=findHelper((Comparable)key,node.leftChild);
	  	}else if(key.compareTo(node.entry.key)>0) {
	  		node=findHelper((Comparable)key,node.rightChild);
   		}else {
   			return node;
   		}
	}
		  ...
		  if(node.leftChild==null&&node.rightChild==null) {
			  ...
			  if(node==root) {
				  ...
				  if(node.equals(node.parent.leftChild)) {
					  ...
					  node.rightChild.parent=node.parent.leftChild;
  ...
  /**helperMethod to find the most left node that contain the
   *  minimum key in subtree with root node:Root
   * @param Root is the root of that subtree.
   * @return BinaryTreeNode contains minimum key value.
   */
 private BinaryTreeNode min(BinaryTreeNode Root) {
	 if(Root!=null) {
		 if(Root.leftChild!=null) {
			 Root=min(Root.leftChild);
		 }else {
			 return Root;
		 }
	 }else {
