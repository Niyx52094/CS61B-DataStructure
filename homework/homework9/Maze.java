...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int numV=vert*(horiz-1);
    int numH=(vert-1)*horiz;
    int w=numV+numH;
    int k=0;
    Wall[] arrayWall=new Wall[w];
    for(i=0;i<vert;i++) {
    	for(j=0;j<horiz-1;j++) {
    	arrayWall[k]=new Wall(j,i,"V");
    	k++;
    	}	
    }
    for(i=0;i<vert-1;i++) {
    	for(j=0;j<horiz;j++) {
    	arrayWall[k]=new Wall(j,i,"H");
    	k++;
    }
    }
	Wall[] temp=new Wall[1];
	//make the array in a random order.
    while(w>1) {
    	int place=randInt(w);
    	temp[0]=arrayWall[place];
    	arrayWall[place]=arrayWall[w-1];
    	arrayWall[w-1]=temp[0];
    	w--;
  }
    
    for(i=0;i<arrayWall.length;i++) {
    	int place=arrayWall[i].vert*horiz+arrayWall[i].horiz;
    	if(arrayWall[i].statement.equals("V")) {
    		int place2=place+1;
    		place=cells.find(place);
    		place2=cells.find(place2);
    		if(cells.find(place)!=cells.find(place2)) {
        		cells.union(place, place2);
        		 vWalls[arrayWall[i].horiz][arrayWall[i].vert] = false;
    		}
    	}else if(arrayWall[i].statement.equals("H")) {
    		int place2=(arrayWall[i].vert+1)*horiz+arrayWall[i].horiz;
    		place=cells.find(place);
    		place2=cells.find(place2);
    		if(cells.find(place)!=cells.find(place2)) {
        		cells.union(place, place2);
        		 hWalls[arrayWall[i].horiz][arrayWall[i].vert] = false;	
    		}
    	}
